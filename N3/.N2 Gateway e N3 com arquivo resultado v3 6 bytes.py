...
# =========== Leitura do pacote UL recebido pela USB  vido do ESP32
      Pacote_UL = ser.read(6) # Checa se chegou o pacote UL

      if len(Pacote_UL) == 6:
# ======= Camada física UL
         # RSSI Dowlink
         print("Rssi UL Byte =", Pacote_UL[3])
         RSSI_DL = ((Pacote_UL[0]-256)/2.0)-74 if Pacote_UL[0] > 128 else (Pacote_UL[0]/2.0)-74

         # RSSI Uplink
         RSSI_UL = ((Pacote_UL[3]-256)/2.0)-74 if Pacote_UL[3] > 128 else (Pacote_UL[3]/2.0)-74

         #SNR Downlink
         Snr_DL = (Pacote_UL[1]*256 + Pacote_UL[2])/10

         #SNR Uplink
         Snr_UL = (Pacote_UL[4]*256 + Pacote_UL[5])/10
...
# ========Camada de aplicação
         ...
         print('Pacote = ',j,' | RSSI DL = ',RSSI_DL,'| RSSI UL = ', RSSI_UL, '| SNR DL =', Snr_DL,'| SNR UL =', Snr_UL)
         print(time.asctime(), ';', j, ';', RSSI_DL, ';', RSSI_UL, Snr_DL, Snr_UL)
